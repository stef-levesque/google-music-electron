...
exports.init = function (gme) {
  ...
  var tray = new Tray(assets['icon-16']);
  ...
  ipcMain.on('change:playback', function handlePlaybackChange (evt, playbackState) {
    ...
    var icon = assets['icon-16'];
    if (playbackState === GMusic.Playback.PLAYING) {
      icon = assets['icon-playing-16'];
    } else if (playbackState === GMusic.Playback.PAUSED) {
      icon = assets['icon-paused-16'];
    }
