...
exports.init = function (gme) {
  ...
  function bindMenuItems(menuItems) {
    menuItems.forEach(function bindMenuItemFn (menuItem) {
      ...
      } else if (cmd === 'window:toggle-miniplayer') {
        menuItem.click = gme.toggleMiniplayer;
      } else if (cmd === 'window:always-on-top') {
        menuItem.click = gme.alwaysOnTop;
      } else if (cmd === 'window:visible-on-all-workspaces') {
        menuItem.click = gme.visibleOnAllWorkspaces;
      } else {
